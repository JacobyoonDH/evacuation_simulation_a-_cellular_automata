...
def visualize_congestion(area_gdf, buildings_gdf, dynamic_congestion_grid, accumulated_congestion_grid, x_coords, y_coords):
    ...
    # 일시적 혼잡도 시각화
    axes[0].imshow(dynamic_congestion_grid, cmap='hot', origin='upper',
    ...
    # 누적 혼잡도 시각화
    axes[1].imshow(accumulated_congestion_grid, cmap='hot', origin='upper',
                   extent=[x_coords[0], x_coords[-1], y_coords[-1], y_coords[0]], alpha=0.6)
...
def visualize_by_types_evacuations(grid, x_coords, y_coords, pedestrians, special_area_gdf, area_gdf, buildings_gdf, evacuation_target_cells):
    """
    보행자들의 대피 경로를 시각화합니다.
    ...
    for pedestrian in pedestrians:
        ...
        if len(path) > 1:
            ...
            # 시작점 마커
            ax.scatter(path_x[0], path_y[0], color=type_color, s=30, marker='o', label="Start" if 'Start' not in ax.get_legend_handles_labels()[1] else "")
            
            # 도착점 마커
            ax.scatter(path_x[-1], path_y[-1], color=type_color, s=30, marker='s', label="End" if 'End' not in ax.get_legend_handles_labels()[1] else "")
    ...
    legend_handles = [
        ...
        mpatches.Patch(color='purple', label='Special Area'),
        mpatches.Patch(color='orange', label='Evacuation Target'),
        mpatches.Patch(color='red', label='노인/어린이'),
        mpatches.Patch(color='blue', label='중장년'),
        mpatches.Patch(color='green', label='청소년/청년'),
        mpatches.Patch(color='magenta', label='장애인')
    ]
...
def visualize_evacuations_by_behavior(grid, x_coords, y_coords, pedestrians, special_area_gdf, area_gdf, buildings_gdf, evacuation_target_cells):
    """
    보행자들의 대피 경로를 행동 패턴별로 시각화합니다.
    ...
    for pedestrian in pedestrians:
        ...
        # 시작점 마커
        ax.scatter(path_x[0], path_y[0], color=behavior_color, s=30, marker='o', label="Start" if 'Start' not in ax.get_legend_handles_labels()[1] else "")

        # 도착점 마커
        ax.scatter(path_x[-1], path_y[-1], color=behavior_color, s=30, marker='s', label="End" if 'End' not in ax.get_legend_handles_labels()[1] else "")

    # 범례 설정
    legend_handles = [
        mpatches.Patch(color='gray', label='도보 대피 가능 지역'),
        mpatches.Patch(color='black', label='건물'),
        mpatches.Patch(color='purple', label='시장'),
        mpatches.Patch(color='orange', label='대피소'),
        mpatches.Patch(color='red', label='특정 대피소 인지'),
        mpatches.Patch(color='blue', label='모든 대피소 인지'),
        mpatches.Patch(color='green', label='대피소 모름')
    ]
